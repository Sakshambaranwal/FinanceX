    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Track stocks, commodities & holdings with live market prices</p>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full sm:w-auto">
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-xl text-sm font-semibold transition-colors disabled:opacity-50 shadow-sm"
            title="Refresh current prices for stocks and commodities"
          >
            <RefreshCw size={16} className={`text-blue-600 ${refreshingPrices ? 'animate-spin' : ''}`} />
            <span>{refreshingPrices ? 'Updating...' : 'Live Quotes'}</span>
          </button>

          <button
            onClick={() => { handleCloseModal(); setIsModalOpen(true); }}
            className="flex-1 sm:flex-none flex items-center justify-center space-x-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-colors shadow-sm text-sm"
          >
            <Plus size={18} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">{investments.length} asset(s) held</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Invested Capital</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">
              {formatCurrency(totalInvested)}
            </p>
            <p className="text-xs text-gray-400 mt-1">Purchase cost basis</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Portfolio P&L</p>
            <div className="flex items-center space-x-2 mt-1">
              <p className={`text-2xl font-extrabold ${totalReturn >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {totalReturn >= 0 ? '+' : '-'}{formatCurrency(Math.abs(totalReturn))}
              </p>
              <span className={`inline-flex items-center text-xs font-bold px-2 py-0.5 rounded-full ${
                totalReturn >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
              }`}>
                {totalReturn >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
                {returnPercentage}%
              </span>
            </div>
            <p className="text-xs text-gray-400 mt-1">Unrealized return on investment</p>
          </div>
          <div className={`p-3 rounded-xl ${totalReturn >= 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            {totalReturn >= 0 ? <TrendingUp size={28} /> : <TrendingDown size={28} />}
          </div>
        </div>
      </div>
      ...
      {categoryList.length > 0 && (
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs text-gray-400">{categoryList.length} categories</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryList.map(cat => {
              ...
              return (
                <div key={cat.name} className="p-4 bg-gray-50/70 rounded-xl border border-gray-200/80 hover:border-gray-300 transition-colors">
                  ...
                  <div className="space-y-1 text-xs text-gray-600 mb-3">
                    <div className="flex justify-between">
                      <span>Invested:</span>
                      <span className="font-semibold text-gray-800">{formatCurrency(cat.invested)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Current Value:</span>
                      <span className="font-semibold text-gray-800">{formatCurrency(cat.current)}</span>
                    </div>
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
          <h3 className="font-bold text-gray-900 text-base">Holdings & Asset Breakdown</h3>
          <span className="text-xs text-gray-500 font-medium">{filteredInvestments.length} position(s)</span>
        </div>
                    ...
                    <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          <p className="font-bold text-gray-900 text-sm">{inv.investmentName}</p>
                          {isMarketAsset && (
                            <span className="inline-flex items-center text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-50 text-blue-700">
                      ...
                      <td className="px-6 py-4 text-right text-gray-600 whitespace-nowrap">
                        {isRD ? (
                          <div>
                            <div className="font-medium text-gray-800">{formatCurrency(depDetails.installmentAmount, invCurr)}</div>
                            <div className="text-[10px] text-gray-400">per {depDetails.frequency === 'Quarterly' ? 'quarter' : depDetails.frequency === 'Half-Yearly' ? 'half-yr' : 'mo'}</div>
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-right text-gray-600 whitespace-nowrap">
                        <div className="font-semibold text-gray-800">{formatCurrency(invested, invCurr)}</div>
                        {isRD && (
                      ...
                      {/* Current Value (Accrued value today) */}
                      <td className="px-6 py-4 text-right font-bold text-gray-900 whitespace-nowrap">
                        <div>{formatCurrency(current, invCurr)}</div>
                        {isDeposit && (
                          <div className="text-[10px] text-gray-400 font-medium">Accrued today</div>
                        )}
                      </td>

                      {/* Maturity Value & Date (Dedicated for FD & RD) */}
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        {isDeposit ? (
                          <div>
                            ...
                            <div className="text-[11px] font-semibold text-gray-600 flex items-center justify-end gap-1 mt-0.5">
                          ...
                          <div className="text-gray-400 text-xs">
                            ...
                            <div className="text-[10px] text-gray-400">Open-ended</div>
                          </div>
                        )}
                      </td>

                      {/* Profit & Loss on EACH investment (Calculated based on Current Accrued Value today) */}
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        {isDeposit ? (
                          <div>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${
                              ret >= 0 ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
                            }`}>
                          ...
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold ${
                            isProfit ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
                          }`}>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <div className={`p-2 rounded-lg ${editingInvestment ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>
            ...
            {!editingInvestment && (
              <div className="mb-4">
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <span className="text-[11px] text-gray-500 font-medium">Validated Market & Bank Data</span>
                </div>
                <div className="space-y-2 bg-gray-50 p-2.5 rounded-xl border border-gray-200">
                  {/* Popular Commodities */}
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[11px] font-bold text-amber-700 w-24">Commodities:</span>
                    {POPULAR_ASSETS.filter(a => a.category === 'Commodities').map(asset => (
                      <button
                        key={asset.symbol}
                        type="button"
                        onClick={() => selectAssetPreset(asset)}
                        className="px-2 py-0.5 text-xs font-semibold bg-white hover:bg-amber-100 hover:text-amber-900 rounded-lg text-amber-800 transition-colors border border-amber-200 shadow-sm"
                      >
                        {asset.symbol}
                      </button>
                    ))}
                  ...
                  {/* Stocks */}
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[11px] font-bold text-blue-700 w-24">Stocks:</span>
                    {POPULAR_ASSETS.filter(a => a.category === 'Stocks').map(asset => (
                      <button
                        key={asset.symbol}
                        type="button"
                        onClick={() => selectAssetPreset(asset)}
                        className="px-2 py-0.5 text-xs font-semibold bg-white hover:bg-blue-100 hover:text-blue-900 rounded-lg text-blue-800 transition-colors border border-blue-200 shadow-sm"
                      >
                        {asset.symbol}
                      </button>
                    ))}
                  ...
                  {/* ETFs & Funds */}
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[11px] font-bold text-indigo-700 w-24">ETFs & Funds:</span>
                    {POPULAR_ASSETS.filter(a => ['Exchange Traded Funds (ETFs)', 'Mutual Funds', 'Bonds'].includes(a.category)).map(asset => (
                      <button
                        key={asset.symbol}
                        type="button"
                        onClick={() => selectAssetPreset(asset)}
                        className="px-2 py-0.5 text-xs font-semibold bg-white hover:bg-indigo-100 hover:text-indigo-900 rounded-lg text-indigo-800 transition-colors border border-indigo-200 shadow-sm"
                      >
                        {asset.symbol}
                      </button>
                    ))}
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap pt-1 border-t border-gray-200">
                    <span className="text-[11px] font-bold text-emerald-700 w-24">Bank Deposits:</span>
                    <button
                      type="button"
                      onClick={() => selectAssetPreset({ category: 'Fixed Deposits', name: 'SBI Fixed Deposit', defaultRoi: '7.10' })}
                      className="px-2.5 py-0.5 text-xs font-bold bg-emerald-100 hover:bg-emerald-200 text-emerald-800 rounded-lg transition-colors border border-emerald-300 shadow-sm"
                    >
                    ...
                    <button
                      ...
                      className="px-2.5 py-0.5 text-xs font-bold bg-teal-100 hover:bg-teal-200 text-teal-800 rounded-lg transition-colors border border-teal-300 shadow-sm"
                    >
            ...
            <form onSubmit={handleAddOrUpdateInvestment} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white font-semibold text-gray-800 disabled:bg-gray-100 disabled:text-gray-500"
                >
                  {INVESTMENT_CATEGORIES.map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
              ...
              {formData.category === 'Fixed Deposits' ? (
                ...
                <div className="space-y-3.5 bg-emerald-50/40 p-4 rounded-2xl border border-emerald-200/80">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-900 uppercase tracking-wider flex items-center gap-1">
                      <Briefcase size={14} className="text-emerald-600" /> Fixed Deposit Parameters
                    </span>
                    <span className="text-[11px] font-medium text-emerald-700">Quarterly Compounding by default</span>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm bg-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-bold text-gray-900 bg-white"
                    />
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-semibold bg-white"
                        />
                        <span className="absolute right-3 top-2 text-xs font-bold text-gray-400">% p.a.</span>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="w-24 px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-semibold bg-white"
                        />
                        <select
                          value={fdData.durationUnit}
                          onChange={(e) => setFdData({ ...fdData, durationUnit: e.target.value })}
                          className="flex-1 px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm bg-white"
                        >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm bg-white font-medium"
                      >
                  ...
                  {fdMaturity.principal > 0 && (
                    <div className="p-3.5 bg-white rounded-xl border border-emerald-300 text-xs space-y-2 shadow-sm">
                      ...
                      <div className="flex justify-between text-gray-600">
                        ...
                        <strong className="text-gray-900">{currencySymbol}{fdMaturity.principal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong>
                      </div>
                      ...
                      <div className="flex justify-between items-center text-gray-900 font-extrabold text-sm pt-2 border-t border-gray-100">
                        <span className="text-emerald-950">Maturity Fund (Payout):</span>
                ...
                <div className="space-y-3.5 bg-teal-50/40 p-4 rounded-2xl border border-teal-200/80">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-teal-900 uppercase tracking-wider flex items-center gap-1">
                      <Briefcase size={14} className="text-teal-600" /> Recurring Deposit Parameters
                    </span>
                    <span className="text-[11px] font-medium text-teal-700">Monthly Frequency & Quarterly Compounding</span>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setRdData({ ...rdData, bankName: e.target.value })}
                      className="w-full px-3.5 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm bg-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm font-bold text-gray-900 bg-white"
                    />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm font-semibold bg-white"
                        />
                        <span className="absolute right-3 top-2 text-xs font-bold text-gray-400">% p.a.</span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm bg-white"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">
                        Deposit Frequency <span className="text-teal-600 font-semibold">(Monthly Default)</span>
                      </label>
                      <select
                        value={rdData.frequency}
                        onChange={(e) => setRdData({ ...rdData, frequency: e.target.value })}
                        className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm bg-white font-semibold text-gray-800"
                      >
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="w-24 px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm font-semibold bg-white"
                        />
                        <select
                          value={rdData.durationUnit}
                          onChange={(e) => setRdData({ ...rdData, durationUnit: e.target.value })}
                          className="flex-1 px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm bg-white"
                        >
                          <option value="Months">Months</option>
                          <option value="Years">Years</option>
                        </select>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm bg-white font-medium"
                    >
                  ...
                  {rdMaturity.installmentAmount > 0 && (
                    <div className="p-3.5 bg-white rounded-xl border border-teal-300 text-xs space-y-2 shadow-sm">
                      <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                        <span className="font-bold text-gray-700 flex items-center gap-1.5">
                      ...
                      <div className="flex justify-between text-gray-600">
                        ...
                        <strong className="text-gray-900">{currencySymbol}{rdMaturity.totalInvested.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong>
                      </div>
                      ...
                      <div className="flex justify-between items-center text-gray-900 font-extrabold text-sm pt-2 border-t border-gray-100">
                        <span className="text-teal-950">Maturity Fund (Payout):</span>
                ...
                <>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      />
                      {fetchingLiveQuote && (
                        <div className="absolute right-3 top-2.5 text-xs text-blue-600 flex items-center gap-1 font-medium animate-pulse">
                    ...
                    {benchmarkInfo && (
                      <div className="mt-2.5 p-2.5 bg-blue-50/90 border border-blue-200 rounded-xl text-xs flex items-center justify-between text-blue-900">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-gray-50 p-3.5 rounded-xl border border-gray-200">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white font-semibold"
                      />
                  ...
                  {formData.principalAmount && (
                    <div className="p-3.5 bg-blue-50 rounded-xl border border-blue-100 text-xs space-y-1.5">
                      <div className="flex justify-between text-gray-700">
                        <span>Total Invested (Cost Basis):</span>
                        <strong className="text-gray-900">{currencySymbol}{formData.principalAmount}</strong>
                      </div>
                      <div className="flex justify-between text-gray-700">
                        <span>Current Market Value:</span>
                        <strong className="text-gray-900">{currencySymbol}{formData.currentAmount || formData.principalAmount}</strong>
                      </div>
                      {formData.currentAmount && (
                        <div className="flex justify-between font-bold pt-1 border-t border-blue-200/60">
                          <span>Expected P&L:</span>
                          <span className={(formData.currentAmount - formData.principalAmount) >= 0 ? 'text-green-600' : 'text-red-600'}>
